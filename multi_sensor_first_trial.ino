...
const int pwmStep = 5;                           // Step value for smoothing transitions
...
// Function: Reads distance (in cm) from sensor at index 'sensorIndex'
float readDistance(int sensorIndex) {
  long duration;
  float distance;
  
  // Trigger the ultrasonic sensor: send a 10 Âµs pulse
  ...
  // Read echo duration with a timeout
...
// Function: Maps the measured distance to a PWM value (0-255) for a single sensor
...
void loop() {
  ...
  // For each additional sensor detecting the object beyond the first, increase intensity by 20%.
  float sizeFactor = 1.0;
  if (countDetected > 1) {
    sizeFactor = 1.0 + (countDetected - 1) * 0.2;
  }
  
  // Step 3: Apply the size factor to each sensor's target PWM (if an object is detected)
  for (int i = 0; i < NUM_SENSORS; i++) {
    if (distances[i] < maxEffectiveDistance) {
  ...
  // Slow down sensor updates to let the user feel the gradual changes in vibration intensity
  delay(15);
}